...
class PodcastRetriever:
    ...
    def __init__(self):
        ...
        self.monthhash = {'Jan': 1, 'Feb': 2, 'Mar': 3, 'Apr': 4, 'May': 5, 'Jun': 6, 'Jul': 7, 'Aug': 8, 'Sep': 9,
        ...
        self.args = None
        self.original_downloaded = None
        self.pod_dir = None
        self.target_file = None
        self.edp = None
        self.ever_down_path = None
    ...
    def download_progress(self, count, block_size, total_size):
        ...
        sys.stdout.write("\r%s ... %d%% " % (self.target_file, percent))
        sys.stdout.flush()

    def download_file(self, url, title, destpath):
        self.target_file = '  Downloading podcast "%s"' % title
        finished = False
        count = self.retries
        while not finished and count > 0:
            try:
                ...
                urllib.request.urlretrieve(url, destpath, reporthook=self.download_progress)
                finished = True
            except Exception as e:
    ...
    @staticmethod
    def wrap_tag(tag, content, extension=''):
        if len(extension) > 0:
            extension = ' ' + extension
...
"""
        ...
        copy_right = self.wrap_tag('center', pagelink + u'&nbsp;Copyright © 2015, P. Lutus')
        page = self.wrap_tag('body', page + copy_right)
    ...
    def main(self):
        ...
        def_date = def_date - timedelta(days=def_days_in_the_past)
        def_year = def_date.year
        def_month = def_date.month

        parser = argparse.ArgumentParser()
        parser.add_argument('-p', '--podcasts', help='Podcast storage directory, default "podcasts"',
                            default='podcasts')
        ...
        parser.add_argument('-m', '--month', help='Earliest month, default 1', type=int, default=def_month)
        ...
        try:
            ...
            for url, podsub in self.resources:
                ...
                for item in channel.findall('item'):
                    ...
                    suffix = re.sub(r'.*\.([a-zA-Z0-9]+).*', r'\1', url)
                    day, month, year = re.sub(r'\S+\s+(\S+)\s+(\S+)\s+(\S+).*', r'\1,\2,\3',
                                              item.find('pubDate').text).split(',')
                    ...
                    if year >= self.args.year and month >= self.args.month and day >= self.args.day:
                        if ((not os.path.exists(destpath) or self.file_size(destpath) == 0) and (
                                dest_tag not in self.ever_downloaded)):
                            ...
                            if os.path.exists(destpath):
                                if self.file_size(destpath) == 0:
                                    ...
                                    t = datetime(year, month, day).timestamp()
